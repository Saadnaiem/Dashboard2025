...
const DivisionDetailView: React.FC<DivisionDetailViewProps> = ({ allRawData }) => {
    ...
    const processedData = useMemo(() => {
        ...
        divisionData.forEach(row => {
            ...
            if (!tableMap.has(key)) tableMap.set(key, { department: row.DEPARTMENT, category: row.CATEGORY, s24: 0, s25: 0, c25: 0 });
            const entry = tableMap.get(key)!;
            entry.s24 += s24; entry.s25 += s25; entry.c25 += row.SALES2025_CASH || 0;
        });
        ...
        return { totalSales24, totalSales25, tableData };
    ...
    const groupedData = useMemo(() => {
        ...
        return Object.entries(groups).map(([dept, cats]) => {
            ...
            const total = cats.reduce((acc, r) => ({ s24: acc.s24 + r.sales2024, s25: acc.s25 + r.sales2025 }), { s24: 0, s25: 0 });
            return { dept, categories: sorted, totalSales24: total.s24, totalSales25: total.s25 };
        }).sort((a, b) => b.totalSales25 - a.totalSales25);
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex justify-between items-center bg-slate-800/40 p-4 rounded-xl border border-slate-700/50">
                <h2 className="text-2xl font-extrabold text-white">Division: <span className="text-sky-400">{divisionName}</span></h2>
                <Link to="/" className="px-4 py-2 bg-sky-600 text-white font-bold rounded-lg text-sm hover:bg-sky-700 transition-all">Back</Link>
            </div>
                ...
                <table className="w-full text-left text-slate-300 table-sortable">
                    <thead className="text-[10px] uppercase bg-slate-900 sticky top-0 z-20 font-bold border-b border-slate-700">
                        <tr>
                            ...
                            <th className="text-slate-400 text-right">Contrib % (25)</th>
                        </tr>
                    </thead>
                        ...
                        {groupedData.map((group, idx) => (
                            <React.Fragment key={group.dept}>
                                <tr className="bg-slate-700/40 font-bold text-xs">
                                    <td className="text-white py-2 uppercase tracking-wide">{group.dept} TOTAL</td>
                                    <td className="text-sky-400 text-right font-numeric">{formatNumberAbbreviated(group.totalSales24)}</td>
                                    <td className="text-green-400 text-right font-numeric">{formatNumberAbbreviated(group.totalSales25)}</td>
                                    <td className="text-right"><GrowthIndicator value={calculateGrowth(group.totalSales25, group.totalSales24)} className="text-xs" /></td>
                                    <td className="text-right font-numeric text-slate-400">{(group.totalSales25/processedData.totalSales25*100).toFixed(1)}%</td>
                                </tr>
                                {group.categories.map((row, cidx) => (
                                    <tr key={cidx} className={`hover:bg-sky-500/10 transition-colors text-xs cursor-pointer ${DEPT_ROW_COLORS[idx % DEPT_ROW_COLORS.length]}`} onClick={() => handleRowClick(group.dept, row.category)}>
                                       <td className="pl-8 text-slate-400 italic">{row.category}</td>
