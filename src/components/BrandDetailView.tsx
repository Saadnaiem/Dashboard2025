...
const BrandDetailView: React.FC<BrandDetailViewProps> = ({ allRawData }) => {
    ...
    const globalFilters = useMemo(() => ({
        ...
        brands: [], // Ignore brand filter from params, as we are viewing a specific brand
        items: [],
    }), [searchParams]);
    ...
    const backLink = `/details/brands?${searchParams.toString()}`;
    ...
    return (
        <div className="flex flex-col gap-6">
             <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <Link to={backLink} className="px-4 py-2 bg-sky-600 text-white font-bold rounded-lg shadow-md hover:bg-sky-700 transition-all flex items-center gap-2">
            ...
            {brandStats && (
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-slate-800/50 p-6 rounded-2xl shadow-xl border border-slate-700 flex flex-col justify-center items-center text-center">
                        ...
                        <div className="text-5xl font-extrabold text-green-400">{brandStats.items25.size}</div>
                         <div className="text-sm font-bold text-slate-400 mt-1">2024: {brandStats.items24.size}</div>
                ...
                <div className="overflow-x-auto p-4">
                    <table className="w-full text-left text-slate-300 table-sortable">
                        <thead className="text-xs text-sky-300 uppercase bg-slate-800 sticky top-0 z-10 font-bold">
                            <tr>
                                {columns.map(col => (
                                    <th key={col.key} scope="col" className={`p-3 whitespace-nowrap ${col.key !== 'no' ? 'cursor-pointer hover:bg-slate-700' : ''} ${col.isNumeric ? 'text-right' : 'text-left'}`} onClick={() => requestSort(col.key as SortableKeys)}>
